...
const RegisterGreenCardForm = () => {
  ...
  const onsubmit = async (data: FormValues) => {
    ...
    if (mode === "add") {
      ...
      changeDialogTitle(Texts.features.soe.grn.registerTitle);
      changeDialogText(Texts.features.soe.grn.confirmModalRegeisterText);
    } else if (mode === "edit") {
      const viewDate = combineDateTimeFields({
        time: data.time,
        date: data.date,
  ...
  return (
    <form
      ...
      className="flex p-4 flex-col w-full gap-4"
    >
      <div
      ...
      >
        <div className={` ${isDesktop ? "w-1/2" : "w-full"}`}>
          <CustomComboBox
            label={Texts.features.soe.grn.unitCode}
            control={control}
            name="unitId"
          ...
          <Controller
            ...
            render={({ field, fieldState }) => (
              <CustomDatePicker
                className="w-full"
                {...field}
                label={Texts.features.soe.grn.seenDate}
                disableFuture
      ...
      >
          ...
          <Controller
            ...
            render={({ field, fieldState }) => (
              <CustomTimePicker
                className="w-full"
                {...field}
                label={Texts.features.soe.grn.seenTime}
                disableFuture
                error={!!fieldState.error}
                helperText={fieldState?.error?.message}
              />
        ...
        <div className={` ${isDesktop ? "w-1/2" : "w-full"}`}>
          <CustomTextInput
            label={Texts.features.soe.grn.placeAdditionalDescription}
            name="placeAdditionalDescription"
            control={control}
          />
          <span className="text-xs text-gray-500 px-4">
            {Texts.features.soe.grn.placeAdditionalDescriptionHelpMSG}{" "}
      ...
      >
        <div className="flex w-full flex-col gap-2">
          <CustomTextInput
            multiline
            rows={3}
            label={Texts.features.soe.grn.placeViewDescription}
            control={control}
            name="placeViewDescription"
          />
          <span className="text-xs text-gray-500 px-4">
            {Texts.features.soe.grn.placeViewDescriptionHelpMSG}{" "}
      ...
      >
        <div className="flex w-full flex-col gap-2">
          <CustomTextInput
            ...
            control={control}
            label={Texts.features.soe.grn.suggestionDescription}
            multiline
            rows={3}
          />
          <span className="text-xs text-gray-500 px-4">
            {Texts.features.soe.grn.suggestionDescriptionHelpMSG}{" "}
      ...
      <div className="flex justify-end w-full py-4 gap-4">
        <CustomButton
          ...
          color="primary"
          variant="contained"
          label={Texts.features.soe.grn.saveAsDraft}
        />
        <CustomButton
          // disabled={isLoading}
          // isLoading={isLoading}
